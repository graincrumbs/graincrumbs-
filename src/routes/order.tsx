...
function OrderPage() {
  ...
  const [form, setForm] = useState({
    ...
    // If user comes from "Order Calendar Cake",
    // Brownie Cake + Calendar Cake are automatically selected.
    ...
    // CHANGED:
    // Month removed completely.
    // Highlighted date is now a real date value.
    calendarDate: "",
  ...
  const toggleBranding = (option: string) => {
    setForm((current) => ({
      ...current,
      corporateBranding: current.corporateBranding.includes(option)
        ? current.corporateBranding.filter((b) => b !== option)
        : [...current.corporateBranding, option],
    }));
  };
  ...
  const waMessage = useMemo(() => {
    const lines = [
      ...
      form.type === "Brownie Cake" &&
        form.calendarCake &&
        form.calendarDate &&
        `Highlighted Date: ${form.calendarDate}`,
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // Calendar cake requires highlighted date.
    if (form.calendarCake && !form.calendarDate) {
      alert("Please select the highlighted date for your Calendar Brownie Cake.");
      return;
    }
      ...
      const orderPayload =
        buildOrderPayload({
          ...
          theme:
            form.type === "Brownie Cake"
              ? form.calendarCake
                ? `Calendar highlighted date: ${form.calendarDate} · Price: ₹${CALENDAR_CAKE_PRICE}`
                : form.theme || null
              : form.type === "Gift Box"
                ? form.giftTheme
                : null,
                      ...
                      {isAssortedBox && (
                        <div className="sm:col-span-2 flex items-start gap-2 rounded-xl border border-[color:var(--gold)]/40 bg-[color:var(--cream-dark)]/50 px-4 py-3 text-sm">
                          ...
                          <p className="text-muted-foreground">
                            ...
                            — 6 pieces per box (one of each flavour).
                            Includes artisanal toppings and decorations.
                            Select how many boxes you'd like below.
                          </p>
                        </div>
                      )}
                  ...
                  <>
                    ...
                    >
                      <label className="flex cursor-pointer items-center gap-2 text-sm">
                        ...
                        📅 Make this a Calendar Brownie Cake
                        (Approx. 1.1kg, fixed price ₹
                        {
                          CALENDAR_CAKE_PRICE
                        })
                      </label>
                    ...
                    {form.calendarCake && (
                      <>
                        ...
                        >
                          ...
                          <p className="mt-2 text-[11px] text-muted-foreground">
                            Select the date you want highlighted on the Calendar Brownie Cake.
                          </p>
                        </Field>
    ...
    return (
      <div className={headerCls}>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-3 inline-flex items-center gap-2 rounded-full bg-[color:var(--gold)] px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--chocolate-dark)] transition-opacity hover:opacity-90"
        >
